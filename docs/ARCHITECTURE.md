...
> **Note:** QuranicWords is local-device-only — there is no sign-in, no remote database, and no leaderboard anywhere in this codebase. Progress round-trips through `BackupRepository`'s local JSON export/import instead of any cloud sync. See [`docs/BACKUP_AND_SYNC.md`](BACKUP_AND_SYNC.md) for how the local backup feature works.
