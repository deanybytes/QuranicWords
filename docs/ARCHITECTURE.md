...
Screens must localize both static UI strings (`stringResource`, resource-qualifier driven — automatic once the `Configuration` is correct) **and** JSON-sourced content fields (`LocalizedText` maps like `title`/`meaning`/`prompt` on entities and `ExerciseContent`, keyed by `Language.tag`) explicitly via `rememberSelectedLanguage()` — the two are separate mechanisms and both must be wired per screen, not just the first one. `LocalizedText.get(language)` falls back to English, then to any entry present, so a not-yet-translated language degrades gracefully rather than showing blank text.
