...
class LessonContentRepeaterTest {
    ...
    @Test
    fun `each word is grouped and repeated independently`() {
        val contents = listOf(
        ...
        val w1Scored = result.filter { it.practicedItemId() == "w1" }
        val w2Scored = result.filter { it.practicedItemId() == "w2" }
        assertEquals(3, w1Scored.size)
        assertEquals(3, w2Scored.size)
    }
